<template>
  <div>
    ...
    <!-- 微信端 -->
    <div class="hidden-sm-and-up" :key="goods.id" v-if="telDis==telIf">
      <!-- 顶部图片及文字 -->
      <div class="wepic">
        <div class="wep-img">
          <img :src="imgCreatedUrl+goods.img" alt="">
  ...
  methods: {
    ...
    join() {
      ...
      this.ajax
        .post(
          "/xinda-api/cart/add",
          this.qs.stringify({ id: this.$route.query.id, num: this.buynum })
        )
        .then(function(data) {
          var jData = data.data;
...
@media all and (max-width: 767px) {
  ...
  .wepic {
    ...
    .wep-cha {
      ...
      color: #fff;
      margin-top: 3.6rem;
      background-color: rgba(128, 120, 108, 0.5);
      position: absolute;
      > div {
        margin-left: 0.3rem;
      }
  ...
  .weservice {
    ...
    .wes-gold {
      ...
      .wes-goldpic {
        ...
        height: 40px;
        display: block;
        background: url("/src/components/images/companyIdstry/m_xbt.png")
          no-repeat -66px -75px;
      }
      .wes-goldcha {
        margin-left: 0.12rem;
